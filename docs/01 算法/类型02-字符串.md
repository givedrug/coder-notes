...
next数组：字符串所有的前缀的最长公共前后缀的长度构成的数组，一般也叫做字符串的next数组。
...
![](assets/类型02-字符串/字符串搜索KMP算法.png)

例如，在上述匹配搜索过程中：

首先求出P = "abaabac"的next数组，上面例子已经求得next数组为[0, 0, 1, 1, 2, 3, 0]。
...
因为之前已经有上一次子串P[0:i]与S对应的部分都相同，所以下一次P的公共前缀，就可以滑到上一次P的公共后缀位置上，这一段无需对比，一定相同，所以可以从P[next[i-1]]处继续匹配。

其二，向后滑动next[i-1]个位置，同时意味着将跳过上一个P[0]和下一个P[0]之间的位置的对比。

比如上述例子中，S[3]对比失败后，P从S[3]处向后滑动1个位置，意味着跳过1个位置S[1]；S[8]对比失败后，P从S[8]处向后滑动3个位置，意味着跳过2个位置S[3]、S[4]。

为什么可以跳过这些位置呢？假设不跳过这些位置（也就是下一次P向后多滑动一些），那下一次P与上一次P的重叠部分一定大于next[i-1]，但next数组已经记录了子串P[0:i]的最大公共前后缀长度了，也就是下一次P前缀和上一次P后缀重叠后相同的最大长度。所以大于这个长度的重叠部分，一定是不相同的。既然一定不相同，那就没有必要对比了。
```

**优化求解next数组**

求解next数组，同样可以使用暴力求解，时间复杂度为$O(m^{2})$。

下面介绍一个快速计算next数组的方法，同样是利用next数组的前面计算结果，来得出当前next的值。

我们已经求出next[x-1]的值，当求子串P[0]~P[x]的next值时，根据当前P[x]值的不同情况可以进行分类处理。

情况1：P[x]与前一个子串的最长公共前缀的下一个字符P[now]相同

如，子串P[0]~P[x-1]的最长公共前（后）缀为"ab"，则now=next[x-1]=2

又有P[x]与P[now]相同，都为"c"，所以next[x]=next[x-1]+1=3

![](assets/类型02-字符串/字符串搜索Next-1.png)
...
要缩小多少呢？回过头来看一下now的定义，即前一个子串的最长前缀的下一个字符的位置，相似的，应该求解子串A的最长前缀的下一个字符的位置，即新的now=next[now-1]。

![](assets/类型02-字符串/字符串搜索Next-2.png)
继续对比P[now]与P[x]，如果相等则next[x]=now+1，如果不相等，则可以继续now=next[now-1]，直到P[now]与P[x]相等或now=0且不相等（此时返回0）。

可以看出情况2和情况1其实是相同的处理。

那么为什么最长可能的公共前缀是从now开始判断呢？可以更长吗？答案和上面分析KMP算法时的思想是类似的，不会更长，因为更长的前后缀对比的话，一定不相同，再长也不会超过now的位置。
