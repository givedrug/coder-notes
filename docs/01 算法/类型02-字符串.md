...
当搜索到S[3]与P[3]时，发现不相等，此时查找next数组，P[3-1]即P[2]对应的next[2]=1，于是可以将模式串P在S[3]处向后滑动1个位置，即将P[1]与S[3]作为起始，向后继续进行匹配；

当搜索到S[8]与P[6]时，发现不相等，此时查找next数组，P[6-1]即P[5]对应的next[5]=3，于是可以将模式串P在S[8]处向后滑动3个位置，即将P[3]与S[8]作为起始，向后继续进行匹配；
...
为什么可以跳过这些位置呢？假设不跳过他们，那下一次P与上一次P的重叠部分一定大于next[i-1]，因为next数组已经记录了下一次P与上一次P最大的公共前后缀长度了，所以大于这个长度的重叠部分，一定是不相同的。既然一定不相同，那就没有必要对比了。
