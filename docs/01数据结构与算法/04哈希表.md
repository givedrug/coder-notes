...
方法一：Java中的LinkedHashMap即是哈希表和链表的结合，因此可以直接通过其实现LRU
...
> 
> lfu.get(1);      // 返回1；cache=\[1,2\], cnt(2)=1, cnt(1)=2
> 
> lfu.put(3, 3);   // 去除键2，因为cnt(2)=1 ，使用计数最小；cache=\[3,1\], cnt(3)=1, cnt(1)=2
> 
> lfu.get(2);      // 返回-1（未找到）
> 
> lfu.get(3);      // 返回3；cache=\[3,1\], cnt(3)=2, cnt(1)=2
> 
> lfu.put(4, 4);   // 去除键1，1和3的cnt相同，但1最久未使用；cache=\[4,3\], cnt(4)=1, cnt(3)=2
> 
> lfu.get(1);      // 返回-1（未找到）
> 
> lfu.get(3);      // 返回3；cache=\[3,4\], cnt(4)=1, cnt(3)=3
> 
> lfu.get(4);      // 返回4；cache=\[3,4\], cnt(4)=2, cnt(3)=3
