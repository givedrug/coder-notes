...
**思路**
- LeetCode 14. 两两获取公共前缀，并使用公共前缀，遍历后续String，最终得到所有的公共前缀
- LeetCode 151. 顺序遍历，跳过空格，遇到非空格后记录单词，并将单词放入list，反序后join空格并返回
- LeetCode 557. 滑动窗口
- （困难）LeetCode 5. 
	方法一：暴力解法，遍历字符，分别将当前字符作为回文中心查找最大回文串（注意奇数回文与偶数回文的差异），复杂度O(n^2)
	方法二：Manacher算法，复杂度O(n)
- （困难）LeetCode 28. 
	方法一：暴力解法，遍历字符串，依次对比，复杂度O(n*m)
	方法二：KMP算法，复杂度O(n+m)
