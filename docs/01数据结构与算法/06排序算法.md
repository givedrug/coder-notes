...
Tim排序是**归并排序**和**二分插入排序**的结合，是一个稳定的排序算法，由Tim Peters于2002年用Python实现。现在，Tim排序是Python的标准排序算法，且被Java SE7等用于对非原始类型的数组排序。因其优秀的性能，号称为“世界上最好的排序算法”（需要警惕任何声称世界最好的东西，后来发展出的很多排序算法似乎有些已经超越Tim排序）。
