...
| 桶排序        | $O(n)$       | $O(n\log n)$或$O(n^2)$ | $O(n)$       | $O(n+k)$ | 外部排序 | 稳定（插入排序等稳定算法） |
| 基数排序       |              |                       |              |          | 外部排序 | 稳定            |

说明：
- $n$表示数据规模，$k$表示数据的最大值减最小值（基数排序），或桶的个数（桶排序）。
...
1. 取得数组中的最大数，并取得位数，即为迭代次数N（例如：数组中最大数值为 1000，则 N=4）；
...
**稳定性**：基数排序基于分别排序，分别收集，相同元素之间顺序永远不变，所以是稳定的。
