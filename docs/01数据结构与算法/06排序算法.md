...
为什么希尔排序可以突破$O(n^2)$的复杂度，获得比插入排序更好的执行效率呢？首先引入，在数组中的两个数字，如果前面一个数字大于后面的数字，则这两个数字组成一个逆序对。一个随机数组，逆序对的数量是$O(n^2)$级别的，对于交换相邻元素的算法，则只能消除一个逆序对，而希尔排序交换了间隔较远的元素，每次消除的逆序对数量也就大于1了，因此效率就得到了提升。其他突破$O(n^2)$级别，达到$O(n \log n)$级别的算法，本质也是因为交换了间隔较远的元素。
...
为什么计数排序可以突破$O(n\log n)$的下界限，而达到$O(n)$级别呢？首先考虑决策树，它是一棵完全二叉树，可以反映比较排序算法中对所有元素的比较操作：

![](assets/06排序算法/sorting-algorithm-decision-tree.png)
...
9. [c++排序函数sort stable_sort底层原理总结](https://zhuanlan.zhihu.com/p/659709757)
10. [世界上最快的排序算法——Timsort](https://www.cnblogs.com/sunshuyi/p/12680918.html)
11. [quadsort](https://github.com/scandum/quadsort)
