...
为什么希尔排序可以获得比插入排序更好的执行效率呢？在数组中的两个数字，如果前面一个数字大于后面的数字，则这两个数字组成一个逆序对。一个随机数组，逆序对的数量是$O(n^2)$级别的，对于交换相邻元素的算法，则只能消除一个逆序对，而希尔排序交换了间隔较远的元素，每次消除的逆序对数量也就大于1了，因此效率就得到了提升。
