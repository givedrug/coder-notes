...
最优情况下，也即k与n相等时，每个桶中只有一个元素，直接有序，故复杂度为$O(n)$。
...
4. [三种快速排序以及快速排序的优化](https://blog.csdn.net/insistGoGo/article/details/7785038)
5. [快速排序和归并排序的时间复杂度分析——通俗易懂](https://www.cnblogs.com/tuyang1129/p/12857821.html)
6. [堆排序的时间和空间复杂度](https://juejin.cn/post/7119807721766912030)

### Tim Sort

Tim排序是**归并排序**和**二分插入排序**的结合，是一个稳定的排序算法，由Tim Peters于2002年用Python实现。现在，Tim排序是Python的标准排序算法，且被Java SE7等用于对非原始类型的数组排序。因其优秀的性能，号称为“世界上最好的排序算法”。

Tim排序在最好情况下的时间复杂度为$O(n)$，最差情况下的时间复杂度为$O(n \log n)$，期望时间复杂度为$O(n \log n)$。Tim排序在最坏情况下的空间复杂度为$O(n)$。
...
解答：通常使用分治+Trie树/Hash+小顶堆。与例子2相似，先将数据集切分（直接切分或使用Hash切分）成多个小数据集，然后使用Trie树或者Hash统计每个小数据集中的词频，之后用小顶堆求出每个数据集中出频率最高的前K个数，最后在所有top K中求出最终的top K个字符串。
