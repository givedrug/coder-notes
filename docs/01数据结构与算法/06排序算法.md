...
| 桶排序        | $O(n)$       | $O(n\log n)$或$O(n^2)$ | $O(n)$       | $O(n+k)$ | 外部排序 | 稳定（插入排序等稳定算法时） |
...
建堆时，会从最后一个非叶子节点开始，依次向上遍历，设共有节点n个，层高为h，则可以推出：
...
综上，建堆操作时间复杂度是$O(n)$，而排序过程时间复杂度是$O(n\log n)$，所以综合时间复杂度（在任何情况下）是$O(n\log n)$。
